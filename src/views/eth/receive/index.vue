      ...
      <v-tabs-items v-model="d_tab">
        <v-tab-item>
          <v-card flat class="pa-6 pt-0 d-flex flex-column justify-start">
            ...
            <v-divider></v-divider>
            <span class="d-flex mt-4 ml-2 mr-2 body-2 text-start">{{ $t('PS: For the BTH currency, only fixed address collection is currently supported. In subsequent versions, we will support obtaining multiple address collections.') }}</span>
          </v-card>
        </v-tab-item>
        <v-tab-item>
...
export default {
  ...
  i18n: {
    messages: {
      zhCN: {
        'PS: For the BTH currency, only fixed address collection is currently supported. In subsequent versions, we will support obtaining multiple address collections.': '备注：针对BTH币种，当前仅支持固定地址收款，后续版本我们将支持获取多个地址收款。',
        'Copy failed!': '复制失败',
